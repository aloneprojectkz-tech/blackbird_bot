...
from blackbird_runner import search_username, search_email
...
def api_search():
    ...
    if search_type not in ("username", "email"):
        return jsonify({"error": "Type must be username or email"}), 400
    ...
    def run():
        ...
        try:
            if search_type == "username":
                ...
                result = loop.run_until_complete(search_email(query, progress_cb))
